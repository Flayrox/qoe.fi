...
describe("@qoe/db - Polls Repository", () => {
  ...
  it("should create a poll with 2 to 4 options", async () => {
    ...
    (prisma.poll.create as any).mockResolvedValue(mockCreated);
  ...
  it("should record a vote and return updated poll results", async () => {
    (prisma.poll.findUnique as any).mockResolvedValueOnce({
      expiresAt: new Date(Date.now() + 3600000),
      thoughtId: "thought-1",
    });
    (prisma.pollVote.create as any).mockResolvedValue({ id: "vote-1" });
    ...
    (prisma.poll.findUnique as any).mockResolvedValueOnce({
      ...
      expiresAt: new Date(Date.now() + 3600000),
      options: [
        { id: "opt-1", text: "Oui", order: 0, _count: { votes: 1 } },
      ],
      votes: [{ optionId: "opt-1" }],
    });
