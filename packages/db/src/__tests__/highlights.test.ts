...
describe("@qoe/db - Highlights & Annotations Repository", () => {
  ...
  it("should create a private highlight successfully", async () => {
    ...
    (prisma.highlight.create as any).mockResolvedValue(mockHighlight);
  ...
  it("should restrict official annotations to the primary article author only", async () => {
    ...
    (prisma.highlight.create as any).mockResolvedValue({});
  ...
  it("should toggle upvote on a public highlight", async () => {
    (prisma.annotationUpvote.findUnique as any).mockResolvedValue(null);
    (prisma.annotationUpvote.create as any).mockResolvedValue({ id: "up-1" });
    (prisma.highlight.update as any).mockResolvedValue({ upvotesCount: 5 });
  ...
  it("should retrieve official, public and user highlights for an article", async () => {
    ...
    (prisma.highlight.findMany as any).mockResolvedValue(mockList);
