...
export const updateCreatorProfileAction = safeAction<any, any>(async (data, user) => {
...
const RESERVED_SUBDOMAINS = [
  "admin", "api", "app", "auth", "billing", "blog", "dashboard", "dev", "developer",
  "docs", "feed", "help", "login", "main", "media", "onboarding", "portal", "qoe",
  "root", "settings", "start", "static", "status", "store", "studio", "support", "www"
];

export const checkSubdomainAvailabilityAction = safeAction<string, { available: boolean; reason?: string }>(
  async (subdomain) => {
    const clean = subdomain.trim().toLowerCase();
    const regex = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
    if (!regex.test(clean)) {
      return { available: false, reason: "Le sous-domaine ne doit contenir que des lettres minuscules, chiffres et tirets." };
    }
    if (clean.length < 3 || clean.length > 30) {
      return { available: false, reason: "La longueur doit être comprise entre 3 et 30 caractères." };
    }
...
export const saveNavigationLinksAction = safeAction<any[], { success: boolean }>(async (links, user) => {
...
export const saveSocialLinksAction = safeAction<any[], { success: boolean }>(async (links, user) => {
  await prisma.$transaction(async (tx: any) => {
...
export const submitApiApplicationAction = safeAction<string, { success: boolean }>(async (reason, user) => {
...
export const completeOnboardingAction = safeAction<any, { success: boolean }>(async (data, user) => {
  await prisma.user.update({
    where: { id: user.id },
    data: {
