...
export class QoeApiClient {
  ...
  public async getFeed(params?: {
    ...
    return this.request<{ items: unknown[]; nextCursor: string | null }>(`/v1/feed${queryString}`);
  }

  public async createThought(
    content: string,
    options?: { imageUrl?: string; triggerWarning?: string; visibility?: 'public' | 'followers' }
  ) {
    return this.request<unknown>('/v1/thoughts', {
      method: 'POST',
  ...
  public async toggleRepost(postId: string) {
    return this.request<{ reposted: boolean; repostsCount: number }>(
      `/v1/thoughts/${postId}/repost`,
      {
        method: 'POST',
      }
    );
  ...
  public async getMyProfile() {
    return this.request<unknown>('/v1/users/me');
  }

  public async getUserProfile(username: string) {
    return this.request<unknown>(`/v1/users/${username}`);
  }
