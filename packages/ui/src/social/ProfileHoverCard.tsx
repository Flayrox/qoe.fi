...
export function ProfileHoverCard({
  ...
  const handle = propUsername || user?.username || user?.subdomain || user?.id?.slice(0, 8) || "user"
    ...
    <HoverCard>
      ...
      <HoverCardContent className="w-72 p-4 bg-popover border border-border/40 rounded-2xl shadow-2xl z-50 font-sans">
        <div className="flex items-start gap-3">
          <AuthorAvatar user={user || { name, username: cleanHandle }} size="md" showBadge={false} />
          <div className="space-y-1 flex-1 min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground leading-none truncate">@{cleanHandle}</p>
