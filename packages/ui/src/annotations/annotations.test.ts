...
describe("@qoe/ui/annotations exports & styling", () => {
  ...
  it("verifies mark styling classes match design specs", () => {
    expect(MARK_STYLE_CLASSES.official).toContain("bg-amber-500/20")
    expect(MARK_STYLE_CLASSES.official).toContain("border-amber-500")
    expect(MARK_STYLE_CLASSES.public).toContain("bg-primary/20")
    expect(MARK_STYLE_CLASSES.public).toContain("border-primary/50")
    expect(MARK_STYLE_CLASSES.private).toContain("bg-amber-500/15")
    expect(MARK_STYLE_CLASSES.private).toContain("border-dashed")
    expect(MARK_STYLE_CLASSES.private).toContain("border-amber-400/60")
  })
  ...
  it("verifies active text spotlight pulse ring classes match design specs", () => {
    ...
    expect(SPOTLIGHT_PULSE_CLASSES).toContain("bg-amber-500/40")
    expect(SPOTLIGHT_PULSE_CLASSES).toContain("shadow-lg shadow-amber-500/30")
  })
})
