...
describe("Empirical Challenge: Subpath Export & TS Contracts Verification", () => {
  ...
  it("2. Exports all required components and visual constants", () => {
    ...
    expect(AnnotationsModule.MARK_STYLE_CLASSES.official).toContain("amber");
  ...
  it("3. TS paths mapping in consuming apps matches subpath export", () => {
    const webTsConfig = JSON.parse(fs.readFileSync(path.join(rootDir, "apps/web/tsconfig.json"), "utf-8"));
    const feedTsConfig = JSON.parse(fs.readFileSync(path.join(rootDir, "apps/feed/tsconfig.json"), "utf-8"));
