...
export interface AnnotationActionCallbacks {
  ...
  onUpvote?: (
    highlightId: string
  ) => Promise<{ ok: boolean; data?: { upvotesCount: number; hasUpvoted: boolean } }>;
  onComment?: (data: {
    highlightId: string;
    content: string;
  }) => Promise<{ ok: boolean; data?: { id: string; content: string; createdAt: string } }>;
  onTogglePrivacy?: (data: {
    highlightId: string;
    isPublic: boolean;
  }) => Promise<{ ok: boolean; data?: { isPublic: boolean } }>;
  onDelete?: (highlightId: string) => Promise<{ ok: boolean }>;
  onUpdateNote?: (data: { highlightId: string; note: string | null }) => Promise<{ ok: boolean }>;
  onCrosspost?: (data: {
    articleId: string;
    text: string;
    commentary?: string;
...
describe('Tier 1: Component Rendering & Action Callback Contract', () => {
  ...
  it('should trigger onUpvote callback when toggling like on an annotation', async () => {
    const onUpvote = vi
      .fn()
      .mockResolvedValue({ ok: true, data: { upvotesCount: 4, hasUpvoted: true } });
