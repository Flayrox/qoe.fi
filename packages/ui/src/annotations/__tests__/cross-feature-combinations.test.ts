...
describe("Tier 3: Cross-Feature Combinations", () => {
    ...
    mockAnnotations = [
      {
        ...
        reader: { id: "author-1", name: "Author", username: "author", logoUrl: null, subdomain: null },
        comments: [
          { id: "c1", content: "Awesome opening!", createdAt: "2026-08-11T10:05:00Z", author: { id: "r1", name: "Reader 1", username: "r1", logoUrl: null } },
        ],
      },
      {
        id: "annot-2",
        ...
        reader: { id: "reader-2", name: "Genius Contributor", username: "genius", logoUrl: null, subdomain: null },
        comments: [],
      },
      {
        id: "annot-3",
  ...
  it("should apply spotlight pulse ring classes on active highlight mark element in DOM", () => {
    ...
    expect(mark.classList.contains("bg-amber-500/40")).toBe(true);
  });

  it("should sort annotations deterministically by DOM offset, length, and creation timestamp", () => {
    const articleText = "First passage in article ... Second passage in article ... Third passage in article";
