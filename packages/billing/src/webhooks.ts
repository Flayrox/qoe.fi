...
export const WEBHOOK_HANDLERS: Record<string, (event: Stripe.Event) => Promise<void>> = {
  ...
  "invoice.payment_succeeded": async (event) => {
    ...
    if (!creatorId || !email) {
      console.warn("⚠️ Impossible de traiter le paiement : métadonnées manquantes dans la facture Stripe.", {
        creatorId,
        email,
      });
      return;
    ...
    if (creator) {
      ...
      console.log(`💰 Créateur ${creatorId} crédité de ${creditAmount / 100}€ (Frais déduits: ${feeCents / 100}€).`);
    }
  },
  ...
  "invoice.payment_failed": async (event) => {
    ...
    if (creatorId && email) {
      ...
      console.log(`🚫 Accès premium révoqué pour ${email} (Abonné au créateur ${creatorId}) suite à un échec de paiement.`);
    }
  },
  ...
  "customer.subscription.deleted": async (event) => {
    ...
    if (creatorId && email) {
      ...
      console.log(`🚫 Abonnement résilié proprement pour ${email} auprès du créateur ${creatorId}.`);
    }
  },
