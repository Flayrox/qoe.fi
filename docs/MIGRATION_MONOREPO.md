...
- **`apps/api`** : backend Go (feed, posts, articles, webhooks, analytics)
...
- **`apps/api`** : backend Go unique sous `api.qoe.fi` (l'ancienne API Hono `apps/api` a été supprimée — voir `SUNSET_API_LEGACY.md`).
