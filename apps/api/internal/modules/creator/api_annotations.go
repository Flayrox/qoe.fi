...
func buildExportAnnotation(doc *canon.Document, row annotationRow) apiExportAnnotation {
	...
	it.ContextBefore = exportContext(doc.Text, start, -1)
	it.ContextAfter = exportContext(doc.Text, end, +1)
	return it
}

// exportContext extrait une fenêtre du texte canonique autour d'un offset
// (code points) : direction -1 = avant, +1 = après. Fenêtre de 40 scalaires
// au plus, tronquée au mot entier, avec « … » si coupée.
func exportContext(text string, at, direction int) string {
	runes := []rune(text)
