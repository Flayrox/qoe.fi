...
import {
  ThoughtThreadProvider,
  type OptimisticThought,
  type ThoughtThreadContextValue,
} from './ThoughtThreadContext';
...
export function ThoughtThreadRoot({
  ...
  useEffect(() => {
    ...
    async function loadThread() {
      ...
      if (res.ok && res.data?.post) {
        ...
        const userHasLiked =
          typeof postData.liked === 'boolean'
            ? postData.liked
            : postData.likes?.some((l: { userId: string }) => l.userId === currentUserId);
        setPost({
          ...postData,
          liked: userHasLiked,
          likesCount:
            typeof postData.likesCount === 'number'
              ? postData.likesCount
  ...
  const handleOpenPost = useCallback(
    (targetPostId: string, authorUsername?: string) => {
      const handle =
        authorUsername ||
  ...
  const handleRepostThought = useCallback(
    async (targetId: string) => {
      if (!currentUserId) {
        if (onLoginRequired) onLoginRequired();
        return;
  ...
  const handleSubmitReply = useCallback(
    async (parentId: string, content: string): Promise<boolean> => {
      ...
      const optimisticReply: OptimisticThought = {
        ...
        author: dbUser
          ? {
              id: dbUser.id || currentUserId || 'unknown',
        ...
        const replaceTempNode = (item: OptimisticThought): OptimisticThought => {
          return {
            ...
            replies: (item.replies || []).map((r) =>
              r.id === tempReplyId ? serverReply : replaceTempNode(r)
            ),
          };
        };
