...
export async function getCreatorAnalyticsData(period: TimePeriod = "30d"): Promise<{ data?: AnalyticsResponseData; error?: string }> {
  try {
    ...
    const [stats, timeseries, topPages, referrers, devices, browsers, countries] = await Promise.all([
    ...
    return { error: err?.message || "Erreur lors de la récupération des données analytics" }
  }
}
 ...
 */
export async function getArticleAnalyticsDetail(urlPath: string, period: TimePeriod = "30d"): Promise<{ data?: ArticleDetailData; error?: string }> {
  try {
    const supabase = await createClient()
    const { data: { user }, error: authError } = await supabase.auth.getUser()
    ...
    const targetWebsiteId = creator.umamiWebsiteId || process.env.NEXT_PUBLIC_UMAMI_WEBSITE_ID || ""
    const matchedArticle = creator.articles.find(a => `/articles/${a.slug}` === urlPath || `/${a.slug}` === urlPath)
    ...
    return { error: err?.message || "Erreur d'inspection" }
  }
}
