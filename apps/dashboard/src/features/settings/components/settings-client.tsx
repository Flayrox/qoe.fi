...
export default function SettingsClient({ initialCreator }: SettingsClientProps) {
  ...
  const [toolbarCoordinates, setToolbarCoordinates] = useState<{ x: number; y: number } | null>(
    null
  );
  ...
  const handlePreviewElementClick = (
    elementId: string,
    tabToOpen: TabType,
    inputRefToFocus: React.RefObject<HTMLInputElement | HTMLTextAreaElement | null> | null,
    ...
    const containerRect = document
      .getElementById('canvas-mockup-container')
    ...
    <div className="relative min-h-[calc(100vh-10rem)] w-full flex flex-col xl:flex-row gap-8">
      ...
      <div className="w-full xl:w-[45%] flex flex-col bg-card border border-border/60 rounded-3xl p-6 shadow-xl h-fit overflow-y-auto">
        <div className="flex items-center justify-between pb-6 border-b border-border/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Modifiez votre site directement ou via les réglages.
              </p>
            </div>
          </div>
          ...
          {hasChanges && (
            <span className="flex items-center gap-1.5 text-xs text-highlight font-medium px-2.5 py-1 rounded-full bg-highlight/10 border border-highlight/20">
        ...
        {activeTab === 'identity' && (
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold flex items-center gap-1.5">
                <AlignLeft className="w-4 h-4 text-muted-foreground" /> Message d'accueil (Hero
                Text)
              </label>
              <textarea
                ...
                <div className="flex items-center justify-between p-3 bg-muted/40 rounded-xl">
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      Laissez les robots de recherche indexer votre profil.
                    </p>
                  </div>
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) =>
                      setFormData((prev) => ({ ...prev, allowIndexing: e.target.checked }))
                    }
                  />
                </div>
              </div>
            ...
            <div className="space-y-2 p-4 border border-border/50 bg-muted/20 rounded-2xl">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] text-muted-foreground">
                  Une couleur unique pour boutons et liens.
                </span>
              </div>
            </div>
        ...
        {activeTab === 'navigation' && (
          <div className="space-y-6">
            ...
            <div className="space-y-2 pt-4 border-t border-border/50">
              ...
              />
              <p className="text-[11px] text-muted-foreground">
                Lien vers votre plateforme de cagnotte ou adhésion (BuyMeACoffee, Patreon, etc.). Un
                bouton "Support Us" s'affichera dans l'en-tête de votre site.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="flex items-center">
                <input
                  ...
                  onChange={(e) =>
                    setSubdomainInput(e.target.value.toLowerCase().replace(/\s+/g, '-'))
                  }
                />
              ...
              <AnimatePresence mode="wait">
                {subdomainCheckResult.loading && (
                  ...
                  >
                    <Loader2 className="w-3.5 h-3.5 animate-spin" /> Vérification de la
                    disponibilité...
                  </motion.div>
                )}
                ...
                {!subdomainCheckResult.loading && subdomainCheckResult.available === true && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0 }}
                    className="flex items-center gap-1.5 text-xs text-success font-bold mt-2 px-1 bg-success/5 border border-success/10 py-1.5 rounded-lg"
                  >
                    <CheckCircle2 className="w-3.5 h-3.5" /> Félicitations ! Ce sous-domaine est
                    libre.
                  </motion.div>
                )}
              ...
              <div className="p-4 bg-primary/5 border border-primary/10 rounded-2xl mt-4">
                ...
                <p className="text-[11px] text-muted-foreground leading-normal">
                  ...
                  Toutes les publications faites sur ce dashboard apparaîtront automatiquement
                  là-bas.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-4 border-t border-border/50">
              <label className="text-sm font-bold block">
                Nom de Domaine Personnalisé (Custom Domain)
              </label>
              <input
                type="text"
                disabled
              ...
              />
              <p className="text-[11px] text-muted-foreground">
                La configuration de votre propre nom de domaine est une fonctionnalité Premium de
                qoe.fi.
              </p>
            </div>
          </div>
      ...
      {/* =====================================================================
          ...
          ===================================================================== */}
      <div className="flex-1 flex flex-col bg-foreground border border-border/80 rounded-3xl p-6 h-[calc(100vh-10rem)] sticky top-20 shadow-2xl overflow-hidden select-none">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-border mb-6">
          ...
          <div className="flex items-center gap-1 bg-popover rounded-lg p-0.5 border border-border">
            <button
        ...
        >
          ...
          <AnimatePresence>
            {activeEditElement && toolbarCoordinates && (
              <motion.div
                ...
                className="flex items-center gap-1.5 bg-popover border border-border p-1.5 rounded-xl shadow-2xl backdrop-blur-md"
              >
                <div className="flex items-center gap-1 px-1.5 py-0.5 border-r border-border">
                  <Sparkles className="w-3.5 h-3.5 text-primary text-highlight" />
                ...
                {activeEditElement === 'banner' && (
                  <button
                    ...
                    className="px-2 py-1 text-[10px] font-bold text-destructive hover:bg-destructive/10 rounded-md flex items-center gap-1"
                  >
                  ...
                  <button
                    onClick={() => {
                      const newName = prompt(
                        "Entrez le nouveau nom d'affichage :",
                        formData.name || ''
                      );
                      if (newName !== null) setFormData((prev) => ({ ...prev, name: newName }));
                    }}
                    className="px-2 py-1 text-[10px] font-bold text-foreground hover:bg-muted rounded-md"
                  >
                    Changer le nom
                  </button>
                )}
                ...
                {activeEditElement === 'supportUrl' && (
                  <button
                    onClick={() => {
                      const newUrl = prompt(
                        "Entrez l'URL de votre cagnotte (BuyMeACoffee/Patreon/etc.) :",
                        formData.supportUrl || ''
                      );
          ...
          <motion.div
            ...
            className={`bg-foreground border border-border shadow-2xl flex flex-col overflow-y-auto transition-colors duration-300 relative ${
              previewDevice === 'mobile'
                ? 'rounded-[2rem] border-4 border-border/80'
                : 'rounded-2xl'
            }`}
          >
            ...
            >
              ...
              <div className="bg-[var(--tenant-bg)] text-[var(--tenant-fg)] min-h-full flex flex-col flex-1 pb-12">
                {/* ───────────────── HEADER AREA ───────────────── */}
                <header
                  className={`border-b border-[var(--tenant-border)] bg-[var(--tenant-card)]/80 backdrop-blur-sm sticky top-0 z-30 transition-all ${
                    formData.layoutStyle === 'brutalist'
                      ? 'border-b-4 border-[var(--tenant-fg)] py-5'
                      : 'py-4'
                  }`}
                >
                  <div className="container mx-auto px-4 flex items-center justify-between">
                    ...
                    <div
                      onClick={(e) =>
                        handlePreviewElementClick('brand', 'identity', nameInputRef, e)
                      }
                      className={`flex items-center gap-2 cursor-pointer edit-element group/brand relative p-1.5 rounded-lg border border-transparent hover:border-dashed hover:border-[var(--tenant-accent)] ${
                        activeEditElement === 'brand'
                          ? 'border-dashed border-[var(--tenant-accent)] bg-[var(--tenant-accent)]/5'
                          : ''
                      }`}
                    >
                      {formData.logoUrl ? (
                        ...
                        <div
                          className="w-8 h-8 rounded-lg bg-[var(--tenant-accent)]/20 flex items-center justify-center font-bold text-xs"
                      ...
                      <span
                        className={`text-base font-black ${formData.layoutStyle === 'brutalist' ? 'uppercase' : ''}`}
                      >
                    ...
                    <nav className="hidden md:flex items-center gap-5 text-xs font-semibold text-[var(--tenant-muted)]">
                      ...
                      <span
                        onClick={(e) =>
                          handlePreviewElementClick('navigation', 'navigation', null, e)
                        }
                    ...
                    {formData.supportUrl && (
                      <div
                        onClick={(e) =>
                          handlePreviewElementClick('supportUrl', 'navigation', null, e)
                        }
                ...
                <section className="relative w-full overflow-hidden border-b border-[var(--tenant-border)]">
                  ...
                  <div
                    onClick={(e) => handlePreviewElementClick('banner', 'identity', null, e)}
                    className={`relative w-full cursor-pointer edit-element group/banner ${
                      formData.headerImageUrl
                        ? 'h-48 md:h-56'
                        : 'h-12 border-b border-[var(--tenant-border)]/40 bg-[var(--tenant-secondary-bg)]'
                  ...
                  <div className="container mx-auto px-4 py-16 md:py-20 text-center max-w-3xl">
                    <div
                      onClick={(e) =>
                        handlePreviewElementClick('heroText', 'identity', heroTextInputRef, e)
                      }
                    ...
                    >
                      ...
                      >
                        {formData.heroText ||
                          `Bienvenue sur le site de ${formData.name || 'mon profil'}`}
                      </h2>
                ...
                <main className="container mx-auto px-4 py-12 max-w-4xl flex-1">
                  <div className="flex items-center justify-between mb-8 border-b border-[var(--tenant-border)] pb-3">
                    <h3
                      className={`text-base font-bold uppercase tracking-wider ${formData.layoutStyle === 'brutalist' ? 'font-black' : ''}`}
                    >
                  ...
                  >
                    {/* Mock article 1 */}
                    <div
                      className={`flex flex-col p-5 bg-[var(--tenant-card)] border border-[var(--tenant-border)] transition-all ${
                        formData.layoutStyle === 'brutalist'
                          ? 'border-4 border-[var(--tenant-fg)] shadow-[4px_4px_0px_0px_var(--tenant-fg)]'
                          : 'rounded-2xl'
                      } ${formData.layoutStyle === 'magazine' ? 'md:col-span-12' : ''}`}
                    >
                      <span className="text-[10px] font-bold uppercase tracking-widest text-[var(--tenant-accent)] mb-2 block">
                        17 JUIN 2026 • TECH
                      </span>
                      <h4
                        className={`text-lg font-bold mb-2 ${formData.layoutStyle === 'brutalist' ? 'uppercase font-black' : ''}`}
                      >
                        L'art de sculpter des interfaces web de niveau art
                      </h4>
                      <p className="text-xs text-[var(--tenant-muted)] leading-relaxed">
                        Exploration des micro-interactions élégantes inspirées de Rauno, de la
                        gestion des spring physics, des animations fluides et de la conception de
                        dashboards créateur uniques...
                      </p>
                    </div>
                    ...
                    >
                      ...
                      <p className="text-xs text-[var(--tenant-muted)] leading-relaxed">
                        Une analyse de l'expérience utilisateur et de l'intégration bidirectionnelle
                        entre l'IA générative et les systèmes de design complexes de demain...
                      </p>
                    </div>
                  </div>
                </main>
                ...
                {/* ───────────────── FOOTER ───────────────── */}
                <footer
                  className={`border-t border-[var(--tenant-border)] bg-[var(--tenant-secondary-bg)] py-12 px-4 text-center mt-auto ${
                    formData.layoutStyle === 'brutalist'
                      ? 'border-t-4 border-[var(--tenant-fg)]'
                      : ''
                  }`}
                >
                  <div className="max-w-md mx-auto space-y-5">
                    ...
                    <p className="text-xs text-[var(--tenant-muted)] leading-normal">
                      {formData.footerText ||
                        'Inscrivez-vous pour recevoir les dernières publications de mon esprit directement dans votre boîte mail.'}
                    </p>
                    ...
                    <div className="flex gap-2 max-w-sm mx-auto">
                      <input
                        ...
                        className={`px-3 py-2 text-xs border bg-[var(--tenant-card)] border-[var(--tenant-border)] flex-1 text-[var(--tenant-muted)] ${
                          formData.layoutStyle === 'brutalist'
                            ? 'border-2 border-[var(--tenant-fg)] font-mono'
                            : 'rounded-xl'
                        }`}
                      />
                      <button
                    ...
                    {formData.socialLinks.length > 0 && (
                      <div className="flex justify-center gap-4 text-[var(--tenant-muted)] pt-6">
                        {formData.socialLinks.map((social, i) => (
                          <span
                            key={i}
                            className="text-xs font-bold uppercase tracking-wider hover:text-[var(--tenant-accent)] transition-colors"
                          >
                            {social.platform}
                          </span>
                        ))}
                    ...
                    <p className="text-[10px] text-[var(--tenant-muted)]/60 pt-6">
                      © {new Date().getFullYear()} {formData.name || 'Mon site'}. Propulsé par
                      qoe.fi
                    </p>
                  </div>
                </footer>
      ...
      <AnimatePresence>
        {hasChanges && (
          <motion.div
            ...
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center justify-between gap-6 px-6 py-4 bg-foreground/90 border border-border text-background rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.4)] backdrop-blur-md w-[90%] max-w-2xl"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-background/70 mt-0.5">
                  Enregistrez pour appliquer ces paramètres sur votre site live.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-2 text-xs font-bold text-background/70 hover:text-background bg-popover hover:bg-popover/80 rounded-xl transition-all cursor-pointer"
              >
                Annuler
              </button>
