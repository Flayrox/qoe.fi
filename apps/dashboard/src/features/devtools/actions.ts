...
export async function getDevtoolsData() {
  try {
    ...
    return { success: false, error: getErrorMessage(error) || 'Unknown database error' };
  }
}
...
export async function createMockUserAction({
  ...
  try {
    ...
    authWarning = getErrorMessage(error) || 'Invalid API Key / Service Role not configured';
...
<p>Le journalisme moderne est mort de sa dépendance aux clics. Pour survivre et retrouver sa dignité, le journalisme doit devenir un sanctuaire pour l'attention du lecteur. Nous ne vendons pas votre cerveau disponible aux publicitaires ; nous construisons ensemble un patrimoine intellectuel commun.</p>`,
    ...
    return {
      success: false,
      error: getErrorMessage(error) || 'Prisma or seeding operation failed',
    };
  }
}
...
export async function resetDatabaseAction() {
  try {
    ...
    return { success: false, error: getErrorMessage(error) || 'Database reset failed' };
  }
}
...
export async function simulateSubscriberAction({
  ...
  try {
    ...
    return { success: false, error: getErrorMessage(error) || 'Subscription simulation failed' };
  }
}
...
export async function simulateFollowAction({
  ...
  try {
    ...
    return { success: false, error: getErrorMessage(error) || 'Follow simulation failed' };
  }
}
...
export async function simulateLikeAction({ postId, userId }: { postId: string; userId: string }) {
  try {
    ...
    return { success: false, error: getErrorMessage(error) || 'Like toggle failed' };
  }
}
...
export async function addMockFundsAction({
  ...
  try {
    ...
    return { success: false, error: getErrorMessage(error) || 'Failed to adjust wallet balance' };
  }
}
...
export async function resetOnboardingAction() {
  try {
    ...
    return { success: false, error: getErrorMessage(error) || 'Failed to reset onboarding' };
  }
}
