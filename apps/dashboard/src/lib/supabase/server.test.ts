...
type CookieOptions = { name: string; value: string; options: { path: string } };
type MockOptions = {
  cookies: {
...
type MockedCreateServerClient = {
  mock: { calls: [unknown, unknown, MockOptions][] };
};
...
describe('createClient', () => {
  ...
  it('should initialize successfully in an Action/Handler Context (writable cookies)', async () => {
    ...
    // Ensure getAll works
    const options = (createServerClient as unknown as MockedCreateServerClient).mock.calls[0][2];
