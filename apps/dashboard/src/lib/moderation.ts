...
export async function moderateImage(file: File | Blob): Promise<{ safe: boolean; reason?: string }> {
  const apiKey = process.env.OPENAI_API_KEY;

  if (!apiKey || apiKey === "sk-mock" || apiKey.startsWith("sk-...")) {
    console.warn("⚠️ OpenAI Moderation: OPENAI_API_KEY is not configured or is mocked. Skipping moderation check.");
    return { safe: true };
  }
