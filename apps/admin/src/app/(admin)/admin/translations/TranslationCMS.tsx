...
interface TranslationCMSProps {
  ...
  initialOverrides: TranslationOverrides;
}

export function TranslationCMS({ defaultFr, defaultEn, initialOverrides }: TranslationCMSProps) {
  const [overrides, setOverrides] = useState<TranslationOverrides>(initialOverrides);
  ...
  const handleResetAll = () => {
    if (
      confirm(
        'Voulez-vous vraiment supprimer TOUTES les surcharges de traduction ? Cela remettra le site dans son état par défaut.'
      )
    ) {
      setOverrides({ fr: {}, en: {} });
      setSaveStatus('idle');
  ...
  const handleExportJSON = () => {
    const dataStr =
      'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(overrides, null, 2));
    const link = document.createElement('a');
      ...
      if (file.name.endsWith('.json')) {
        try {
          ...
          if (parsed && (parsed.fr || parsed.en)) {
            ...
            setSaveStatus('idle');
            alert(
              'Surcharges de traduction JSON chargées avec succès ! Cliquez sur Enregistrer pour appliquer.'
            );
          } else {
        ...
        try {
          ...
          setSaveStatus('idle');
          alert(
            'Fichier de traduction CSV chargé avec succès ! Cliquez sur Enregistrer pour appliquer.'
          );
        } catch {
  ...
  const overriddenCount =
    Object.keys(overrides.fr || {}).length + Object.keys(overrides.en || {}).length;

  return (
    <div className="space-y-6 max-w-6xl mx-auto font-sans text-foreground pb-16">
      {/* Top Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-border pb-4 gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1 text-[11px] text-muted-foreground font-bold border border-border px-3 py-1.5 rounded-lg hover:bg-muted transition-all cursor-pointer"
          >
          ...
          <label className="inline-flex items-center gap-1 text-[11px] text-muted-foreground font-bold border border-border px-3 py-1.5 rounded-lg hover:bg-muted transition-all cursor-pointer">
          ...
          {overriddenCount > 0 && (
            <button
              ...
              className="inline-flex items-center gap-1 text-[11px] text-muted-foreground hover:text-destructive font-bold border border-border px-3 py-1.5 rounded-lg hover:bg-destructive/10 hover:border-destructive transition-all cursor-pointer"
            >
      ...
      {/* Dense Controls Row */}
      <div className="bg-white border border-border rounded-xl p-3.5 flex flex-col md:flex-row gap-3 items-center">
        {/* Search */}
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full bg-muted border border-border rounded-lg pl-8.5 pr-3 py-1.5 text-xs focus:ring-1 focus:ring-[#EE4B2B] focus:border-[#EE4B2B] outline-none"
          />
        </div>

        {/* Namespace Select */}
        <div className="flex items-center gap-1.5 w-full md:w-auto shrink-0">
          <Filter className="w-3.5 h-3.5 text-muted-foreground hidden md:block" />
          <select
            value={selectedNamespace}
            onChange={(e) => setSelectedNamespace(e.target.value)}
            className="w-full md:w-40 bg-muted border border-border rounded-lg px-2.5 py-1.5 text-xs font-bold focus:ring-1 focus:ring-[#EE4B2B] focus:border-[#EE4B2B] outline-none cursor-pointer"
          >
      ...
      {saveStatus !== 'idle' && (
        <div
          className={`p-3 rounded-lg border flex items-center gap-2 text-[11px] font-bold ${
            saveStatus === 'success'
              ? 'bg-success/10 border-success text-success'
              : 'bg-destructive/10 border-destructive text-[#EE4B2B]'
          }`}
        >
      ...
      <div className="border border-border rounded-xl overflow-hidden bg-white shadow-xs">
        {/* Table Header */}
        <div className="grid grid-cols-12 gap-4 bg-muted border-b border-border px-4 py-2 text-[10px] uppercase font-bold text-muted-foreground select-none">
      ...
      {isDirty && (
        <div className="fixed bottom-6 right-6 z-40 bg-white border border-border rounded-2xl px-5 py-3 shadow-xl flex items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
          <div className="flex flex-col gap-0.5">
            <span className="text-xs font-bold text-foreground">Modifications en attente</span>
          ...
          <div className="bg-white rounded-2xl border border-border shadow-2xl w-full max-w-xl p-5 space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  Vérifiez la liste de vos changements avant de les appliquer en ligne :
                </p>
              </div>
              <button
                type="button"
                onClick={() => setShowModal(false)}
                className="p-1 rounded hover:bg-secondary text-muted-foreground hover:text-muted-foreground transition-colors cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            {/* List of changes */}
            <div className="max-h-64 overflow-y-auto divide-y divide-border border border-border/80 rounded-xl p-3 bg-muted/50">
              {diff.map((item, index) => (
                <div key={index} className="py-2 first:pt-0 last:pb-0 text-xs flex flex-col gap-1">
                  <div className="flex justify-between items-center gap-2">
                    <span className="font-semibold text-foreground truncate break-all pr-4">
                      {item.key}
                    </span>
                    <span className="text-[8px] uppercase font-bold px-1.5 py-0.5 rounded bg-muted text-muted-foreground shrink-0">
                      {item.lang === 'fr' ? '🇫🇷 FR' : '🇬🇧 EN'}
                    </span>
                  </div>
                  <div className="text-[10px] text-muted-foreground flex items-center gap-1.5 flex-wrap">
                    <span
                      className="line-through text-muted-foreground truncate max-w-[200px]"
                      title={item.oldVal}
                    >
                      {item.oldVal}
                    </span>
                    <span>→</span>
                    <span
                      className="font-bold text-[#EE4B2B] truncate max-w-[240px]"
                      title={item.newVal}
                    >
                      {item.newVal}
                    </span>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-border">
              <button
                type="button"
                onClick={() => setShowModal(false)}
                className="px-3.5 py-2 rounded-lg border border-border hover:bg-muted text-xs font-semibold cursor-pointer text-muted-foreground"
              >
                Annuler
              </button>
              <button
