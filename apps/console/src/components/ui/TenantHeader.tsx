...
export function TenantHeader({
  ...
  const nestedNav = topLevelNav.map(parent => ({
    ...parent,
    children: navigation.filter(n => n.parentId === parent.id).sort((a, b) => a.order - b.order)
  })).sort((a, b) => a.order - b.order);

  return (
    <header className={headerClasses}>
      <div className={`container mx-auto px-4 lg:px-8 flex items-center justify-between ${isArticlePage ? 'max-w-5xl' : ''}`}>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="/" className="flex items-center gap-4 group">
            ...
            <h1 className={`text-xl md:text-2xl lg:text-3xl tracking-tight ${isBrutalist ? 'font-black uppercase' : 'font-extrabold'}`}>
              {name || domain}
            </h1>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-6">
                ...
                return (
                  <div key={nav.id} className="relative group/nav">
                    ...
                    <div className="absolute top-full left-0 pt-2 opacity-0 translate-y-2 pointer-events-none group-hover/nav:opacity-100 group-hover/nav:translate-y-0 group-hover/nav:pointer-events-auto transition-all duration-200 w-48 z-50">
                      <div className={`flex flex-col overflow-hidden ${isBrutalist ? 'border-2 border-foreground bg-background shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]' : 'bg-card border rounded-xl shadow-lg'}`}>
          ...
          <div className="flex items-center gap-4 border-l pl-4 md:pl-6 ml-2">
            {socialLinks.slice(0, 3).map((social: any) => (
              <Link 
                key={social.id} 
                href={("/" as any) as any} 
                target="_blank" 
