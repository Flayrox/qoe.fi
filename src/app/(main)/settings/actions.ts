...
export async function exportUserData() {
  ...
  try {
    ...
    const exportData = {
      ...
      interactions: {
        following: dbUser.following.map(f => ({
          creatorName: f.creator.name,
          creatorSubdomain: f.creator.subdomain,
          followedAt: f.createdAt
        })),
        bookmarks: dbUser.bookmarks.map(b => ({
          articleTitle: b.article.title,
          articleSlug: b.article.slug,
          bookmarkedAt: b.createdAt
        })),
        highlights: dbUser.highlights.map(h => ({
          articleTitle: h.article.title,
          quoteText: h.text,
          personalNote: h.note,
          highlightedAt: h.createdAt
        })),
        walletTransactions: dbUser.walletTransactions.map(t => ({
          amountCents: t.amountCents,
          type: t.type,
          timestamp: t.createdAt
        })),
        microPosts: dbUser.posts.map(p => ({
          content: p.content,
          tags: p.tags,
          createdAt: p.createdAt
        })),
        sentLetters: dbUser.sentLetters.map(l => ({
          content: l.content,
          isPublic: l.isPublic,
          sentAt: l.createdAt
        }))
