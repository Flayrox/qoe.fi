...
test.describe("Tier 4: E2E Annotation Engine — Layout, Boundaries, Rauno Physics & Tenant Styling", () => {
  ...
  test.describe("R2: Feed Reader Drawer (94vh & md:left-64 Sidebar Boundary)", () => {
    test("should maintain h-[94vh] height and preserve md:left-64 sidebar navigation boundary", async ({ page }) => {
    ...
    test("should lock body scroll when drawer opens and release on ESC keypress", async ({ page }) => {
      await page.setContent(`
        <html>
          <body>
  ...
  test.describe("R3: Tenant Creator Page Compatibility & Accent Styling", () => {
    test("should respect --tenant-accent CSS variable for creator highlighting", async ({ page }) => {
      await page.setContent(`
        <html>
          <head>
            <style>
    ...
    test("should maintain paywall isolation by slicing content at paywall boundary", async ({ page }) => {
      ...
      const fullContent = "Free preview passage. <!-- PAYWALL --> Locked premium subscriber passage.";
  ...
  test.describe("R1: Rauno Selection Toolbar & Spring Physics Surface", () => {
    test("should render morphing toolbar with layoutId='rauno-morphing-surface' and spring physics", async ({ page }) => {
      await page.setContent(`
        <html>
          <body>
