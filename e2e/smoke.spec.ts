...
test.describe("Linear-Grade E2E Scan & Zero-Hydration Error Audit", () => {
  for (const route of ROUTES) {
    test(`route "${route}" should render cleanly without console or hydration errors`, async ({ page }) => {
      ...
      expect(hydrationErrors, `Hydration errors detected on route ${route}: ${hydrationErrors.join(", ")}`).toEqual([]);
      expect(pageErrors, `Unhandled page errors detected on route ${route}: ${pageErrors.join(", ")}`).toEqual([]);
    });
  }
});
